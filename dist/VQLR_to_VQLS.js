import { flattenObject, renameKeys } from "./utils.js";
export function convert_VQLR_to_VQLS(res) {
    let string = "";
    if (!("d" in res)) {
    ...
    if (baseOp === "find") { }
    else if (baseOp === "update")
        collection = "~" + collection;
    else if (baseOp === "remove")
        collection = "-" + collection;
    else if (baseOp === "add")
        collection = "+" + collection;
    else if (opName === "updateOneOrAdd")
        collection = "updateOneOrAdd " + collection;
    if (opName.endsWith("One"))
        collection += "!";
